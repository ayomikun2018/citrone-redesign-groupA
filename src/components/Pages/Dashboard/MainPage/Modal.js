...
const ModalItem = ({ image, percent, performance, comment, one, two, submitPercent, comment2, number, number2, task }) => {
  ...
  return (
    <React.Fragment>
      <div></div>
      <div
        className="relative z-0 flex w-4/5 h-20 -ml-4 mt-6 border border-gray-300 drop-shadow bg-[#fbfbfb] rounded-xl hover:bg-[#f1f1f1] cursor-pointer hover:transition-all duration-300 ease-in-out hover:duration-700 hover:delay-500"
      ...
      >
        ...
        <p className="flex items-center font-normal ml-10" style={{ fontSize: '16px' }}>
          {performance}
        </p>

        {showModal && (
          <div className="fixed z-50 transition-all duration-700 delay-500 modal">
            ...
            <div className="p-2 ml-6 -mt-2">
              <p className="leading-normal text-base">
                {one}
                <span className="text-[#f64f59]">{percent}</span>
              </p>
              <p className="leading-normal text-base">
                {two}
                <span className="text-[#f64f59]">{submitPercent}</span>
              </p>
              <p className="leading-normal text-base">
                {task}
                <span className="text-[#f64f59]">{number}</span>
              </p>
